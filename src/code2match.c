/* Last change: 2025/06/06-09:16:34.
...
int untie_match(int pos) {
  ...
  for (i = 0; i < opnum; i++)
    if (opptr[i].hin == pos) {
      if (opptr[i].e_pair >= 0 && opptr[i].e_pair < ctnum) {
  ...
  for (i = 0; i < opnum; i++)
    if (opptr[i].e_pair != NOPAIR && opptr[i].e_valid == VALID)
      printf("%2d %2d %2d %24s %02d <-> %02d %s\n", k++, opptr[i].e_size,
             opptr[i].hin, opptr[i].kan, i, opptr[i].e_pair,
      ...
      if (found) {
        ...
        if (!inline_out) {
          ...
          printf("%2d %2d %2d %2d %2d %2d %2d %2d  %s  %s  %s\n", i,
                 opptr[i].sub, opptr[i].e_valid, opptr[i].e_size,
                 opptr[i].e_pair, opptr[i].hin, opptr[i].type, opptr[i].form,
                 opptr[i].sid, opptr[i].sur, opptr[i].kan);
          ...
          for (j = m; j < n; j++)
            if (ctptr[j].e_valid == 1) {
              if (ctptr[j].sub > 1) {
                ...
                if (prev_out != k) {
                  printf("%2d %2d %2d %2d %2d %2d %2d %2d  %s  %s  %s\n", k,
                         ctptr[k].sub, ctptr[k].e_valid, ctptr[k].e_size,
                         ctptr[k].e_pair, ctptr[k].hin, ctptr[k].type,
                         ctptr[k].form, ctptr[k].sid, ctptr[k].sur,
                         ctptr[k].kan);
                  prev_out = k;
                }
              } else {
                printf("%2d %2d %2d %2d %2d %2d %2d %2d  %s  %s  %s\n", j,
                       ctptr[j].sub, ctptr[j].e_valid, ctptr[j].e_size,
                       ctptr[j].e_pair, ctptr[j].hin, ctptr[j].type,
                       ctptr[j].form, ctptr[j].sid, ctptr[j].sur, ctptr[j].kan);
                prev_out = j;
              }
            }
        }
...
int b_word(int num) {
  ...
  if (num == 74 || num == 69 || num == 49 || num == 64 || num == 48 ||
      num == 52 || num == 47)
    r = TRUE;
  return r;
}
...
int show_residual(CCC *ptr, int num) {
  ...
  for (i = 0; i < num; i++)
    if (ptr[i].e_valid == 1 && ptr[i].e_pair < 0) {
      if (inline_out) {
        printf("RESIDUAL %d %s %2d %s %s\n", poem_number, author, ptr[i].hin,
               ptr[i].sid, ptr[i].kan);
      } else {
        printf("%2d %2d %2d %2d %2d %2d %2d %2d  %s  %s  %s\n", i, ptr[i].sub,
               ptr[i].e_valid, ptr[i].e_size, ptr[i].e_pair, ptr[i].hin,
               ptr[i].type, ptr[i].form, ptr[i].sid, ptr[i].sur, ptr[i].kan);
      }
    }
  return 0;
}
...
int get_op_number(CCC *rstr) { return rstr[0].num; }

char *get_ct_author(CCC *rstr) { return rstr[0].name; }
...
int browse(CCC *rstr, int num) {
  ...
  for (i = 0; i < num; i++) {
    printf("%2d %2d %2d %2d %2d %2d %2d %2d  %s %s %s", i, rstr[i].sub,
           rstr[i].e_valid, rstr[i].e_size, rstr[i].e_pair, rstr[i].hin,
           rstr[i].type, rstr[i].form, rstr[i].sid, rstr[i].sur, rstr[i].kan);
...
int line2token(char *istr, CCC *rstr) {
  ...
  strncpy(rstr->name, ptr, sizeof(rstr->name) - 1);
  rstr->name[sizeof(rstr->name) - 1] = '\0';
  ptr = strtok(NULL, " ");
  ...
  //  strcpy(rstr->sid, ptr);
  strncpy(rstr->sid, ptr, sizeof(rstr->sid) - 1);
  rstr->sid[sizeof(rstr->sid) - 1] = '\0';
  ptr = strtok(NULL, " ");

  // strcpy(rstr->sur, ptr);
  strncpy(rstr->sur, ptr, sizeof(rstr->sur) - 1);
  rstr->sur[sizeof(rstr->sur) - 1] = '\0';
  ptr = strtok(NULL, " ");

  //  strcpy(rstr->hir, ptr);
  strncpy(rstr->hir, ptr, sizeof(rstr->hir) - 1);
  rstr->hir[sizeof(rstr->hir) - 1] = '\0';
  ptr = strtok(NULL, " ");

  //  strcpy(rstr->kan, ptr);
  strncpy(rstr->kan, ptr, sizeof(rstr->kan) - 1);
  rstr->kan[sizeof(rstr->kan) - 1] = '\0';
...
int main(int argc, char **argv) {
  ...
  opptr = (CCC *)calloc(i + 1, sizeof(CCC));
  if (opptr == NULL) {
    perror("calloc");
    exit(EXIT_FAILURE);
  }

  ctptr = (CCC *)calloc(k + 1, sizeof(CCC));
  if (ctptr == NULL) {
    perror("calloc");
    exit(EXIT_FAILURE);
  }
